...
class Display extends Stand {
    ...
    constructor(name, options, scene) {
        ...
        if (options.windowOpened && MODE !== "screenshot" && MODE !== "cinematic" && !DEST)
        ...
        if (options.vrSide !== undefined) this.vrSide = options.vrSide;
        ...
        if (this.sound) {
            ...
            const soundLink = this.soundLink;
            scene.highlight(soundLink);
            ...
            this.soundLink.setEnabled(false);
        ...
        if (this.description) {
            ...
            /* main window */
            const standWindow = BABYLON.MeshBuilder.CreatePlane("standWindow", {
                height: 0.3,
                width: this.width,
                sideOrientation: BABYLON.Mesh.DOUBLESIDE,
            });
            ...
            this.standWindow.setEnabled(false);
            this.standWindowBack.isVisible = false;
            this.standWindowBack.setEnabled(false);
            ...
            const standWindowTitleMini = new BABYLON.GUI.TextBlock();
            standWindowTitleMini.text = "⇧ " + this.title + " ⇧";
            standWindowTitleMini.fontFamily = this.scene.fontName;
            standWindowTitleMini.color = "white";
            standWindowTitleMini.fontSize = 23;
            standWindowTitleMini.verticalAlignment = BABYLON.GUI.Control.VERTICAL_ALIGNMENT_CENTER;
            ...
            this.standWindowTexturemin.addControl(standWindowTitleMini);
            this._materialMin = this.standWindow.material;
            ...
            this._materialFull = this.standWindow.material;
            this.standWindow.material = this._materialMin;
            this.standWindow.position = new BABYLON.Vector3(
                0,
                this.height / 2 + 0.05,
                -positionFront,
            ...
            const actionManager = scene.highlight(standWindow);
     ...
     */
    setVRSide() {
        this.plinth.position.x = this.vrSide !== undefined ? this.vrSide : -0.3;
        this.plinth.position.z += 0.1;
    }

    windowSwitch(force) {
        let positionFront = 0.03;
        if (this.depth >= 0.3) positionFront = this.depth / 2 + 0.005;
        const animationBox1 = new BABYLON.Animation(
            "boxAnimation",
            "scaling.y",
            30,
        ...
        const animationBox2 = new BABYLON.Animation(
            "boxAnimation2",
            "position.y",
            30,
            BABYLON.Animation.ANIMATIONTYPE_FLOAT,
        ...
        if (this.standWindowBack.scaling.y == 1) {
            ...
            if (this.animationActivated == false) {
                ...
                this.standWindow.isVisible = false;
                const standWindow = this.standWindow;
                const event = new BABYLON.AnimationEvent(
                    10,
                    function () {
            ...
            if (this.animationActivated == false) {
                ...
                this.standWindow.material = this._materialFull;
                this.standWindowTitle.text = this.title;
                this.standWindow.isVisible = true;
            } else {
                const keys = [];
                ...
                this.standWindowBack.anim.onAnimationEnd = function () {
                    standWindow.material = display._materialFull;
                    display.standWindowTitle.text = display.title;
                    standWindow.isVisible = true;
                };
            }
        }
    ...
    doWhenLeaving() {
        ...
        if (this.windowOpened && this.description && this.standWindowBack && this.standWindowBack.scaling.y == 1) {
            super.doWhenLeaving();
            this.standWindow.setEnabled(true);
            this.standWindowBack.setEnabled(true);
            this.standWindow.isVisible = true;
            this.standWindowBack.isVisible = true;
            this.standWindowBack.isPickable = false;
            this.standWindow.isVisible = false;
            const animScale = new BABYLON.Animation("winClose1", "scaling.y", 30, BABYLON.Animation.ANIMATIONTYPE_FLOAT, BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT);
            const animPos = new BABYLON.Animation("winClose2", "position.y", 30, BABYLON.Animation.ANIMATIONTYPE_FLOAT, BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT);
            animScale.setKeys([{ frame: 0, value: 1 }, { frame: 10, value: 0.2 }]);
            animPos.setKeys([{ frame: 0, value: this.height / 2 + 0.3 }, { frame: 10, value: this.height / 2 + 0.05 }]);
            this.standWindowBack.animations = [animScale, animPos];
            const anim = this.scene.beginAnimation(this.standWindowBack, 0, 10, false);
            anim.onAnimationEnd = () => {
                this.standWindowBack.isVisible = false;
                this.standWindowBack.setEnabled(false);
                this.standWindow.isVisible = false;
                this.standWindow.setEnabled(false);
                this.standWindowBack.scaling.y = 0.2;
                this.standWindowBack.position.y = this.height / 2 + 0.05;
                this.standWindow.scaling.y = 0.2;
                this.standWindow.position.y = this.height / 2 + 0.05;
                this.standWindow.material = this._materialMin;
            };
        } else {
            super.doWhenLeaving();
    ...
    doWhenArriving(openDelay = 600) {
        super.doWhenArriving();
        if (this.windowOpened && this.description) {
            let positionFront = 0.03;
            if (this.depth >= 0.3) positionFront = this.depth / 2 + 0.005;
            this.standWindow.isVisible = false;
            this.standWindowBack.isVisible = false;
            ...
            this._openTimeout = setTimeout(() => {
                ...
                const animScale = new BABYLON.Animation("winOpen1", "scaling.y", 30, BABYLON.Animation.ANIMATIONTYPE_FLOAT, BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT);
                const animPos = new BABYLON.Animation("winOpen2", "position.y", 30, BABYLON.Animation.ANIMATIONTYPE_FLOAT, BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT);
                animScale.setKeys([{ frame: 0, value: 0.2 }, { frame: 10, value: 1 }]);
                animPos.setKeys([{ frame: 0, value: this.height / 2 + 0.05 }, { frame: 10, value: this.height / 2 + 0.3 }]);
                this.standWindowBack.animations = [animScale, animPos];
                const anim = this.scene.beginAnimation(this.standWindowBack, 0, 10, false);
                anim.onAnimationEnd = () => {
                    this.standWindow.material = this._materialFull;
